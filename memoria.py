...
_requested_db = os.getenv('DB_PATH','loteria_master_ai.db')
try:
    os.makedirs(os.path.dirname(_requested_db) or '.', exist_ok=True)
    test_dir=os.path.dirname(_requested_db) or '.'
    if not os.access(test_dir, os.W_OK):
        raise PermissionError(test_dir)
