...
from memoria import guardar_captura,guardar_verificado,guardar_estado
...
def verificar_sala(clave,fecha=None):
    ...
    fecha_confirmada=any(x.get('fecha_fuente')==fecha for x in valid)
    if len(valid)>=2 and valid[0]['resultado']==valid[1]['resultado'] and fecha_confirmada:
        result=valid[0]['resultado']; fuentes=[x['fuente'] for x in valid]; guardar_verificado('rd',clave,nombre,tipo,fecha,result,'VERIFICADO',fuentes)
        return {'estado':'VERIFICADO','guardado':True,'resultado':result,'fuentes':fuentes}
    return {'estado':'PENDIENTE','guardado':False,'mensaje':'No entra en memoria hasta que dos fuentes coincidan y la fecha sea correcta.','fuentes':[a,b]}
