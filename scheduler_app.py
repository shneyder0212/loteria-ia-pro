...
def _bootstrap_inmediato():
    # Small, safe startup job: current RD sync + small history batch.
    try:
        fecha=datetime.now(TZ_RD).date().isoformat()
        sincronizar_todas(fecha,region='rd')
    except Exception as e:
        print(f'[bootstrap] sync actual: {e}')
    try:
        cargar_historico_incremental(4,4)
    except Exception as e:
        print(f'[bootstrap] historico: {e}')
...
def iniciar_scheduler():
    ...
    threading.Thread(target=_bootstrap_inmediato,name='shneyder-bootstrap',daemon=True).start()
