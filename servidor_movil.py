...
def panel(clave,tipo):
    ...
    muestras=d.get('muestras',0)
    etiqueta={
        'MEMORIA_MINIMA':'🟠 MEMORIA MÍNIMA',
        'PROVISIONAL':'🟡 ESTUDIO PROVISIONAL',
        'COMPLETO':'🟢 ESTUDIO COMPLETO',
        'MADURO':'🔵 MEMORIA MADURA'
    }.get(modo,modo)
    modo_html=f"<div class='box'><b>{etiqueta}</b><br>Muestras verificadas: {muestras} · Confianza del modelo: {confianza}<br><small>La app seguirá acumulando resultados y recalculando el estudio al paso.</small></div>"
    alerta_html=''
    if tipo=='quiniela':
    ...
    return f"{modo_html}{alerta_html}{especial}<div class='box'><b>Memoria verificada:</b> {d['muestras']} sorteos.</div>{bt_html}<table><tr><th>#</th><th>Número</th><th>Score</th><th>Razones</th></tr>{rows}</table>"
