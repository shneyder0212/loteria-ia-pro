...
from memoria import inicializar_db,contar,contar_observados,contar_trabajo,resumen_memoria_clave,listar_predicciones_fecha,resumen_estado,leer_estado,ultimo_resultado
...
from bootstrap import importar_bootstrap

app=FastAPI(title='Shneyder IA Pro - Estable Real')
inicializar_db(); MAPA={x[0]:x for x in SALAS}


@app.on_event('startup')
def startup_app():
    # Local bootstrap first: fast, no internet dependency.
    importar_bootstrap()
    # Network/scheduler work starts in background only after the app exists.
    iniciar_scheduler()

@app.get('/api/version',response_class=JSONResponse)
def api_version():
    return {'version':'ESTABLE_REAL_v1','modo':'DESDE_EL_PRIMER_DATO'}

@app.get('/api/bootstrap',response_class=JSONResponse)
def api_bootstrap():
    return importar_bootstrap()

@app.get('/ping',response_class=PlainTextResponse)
def ping(): return 'OK - Estable Real'

@app.get('/api/arrancar',response_class=JSONResponse)
def api_arrancar():
    actual=sincronizar_todas()
    historico=cargar_historico_incremental(6,6)
    progreso={}
    for clave,nombre,tipo,region in SALAS:
        progreso[clave]=resumen_memoria_clave(clave)
    return {'sincronizacion':actual,'historico':historico,'progreso':progreso}

@app.get('/api/progreso',response_class=JSONResponse)
def api_progreso():
    salida={}
    for clave,nombre,tipo,region in SALAS:
        d=_analisis(clave,tipo)
        mem=resumen_memoria_clave(clave)
        salida[clave]={
            'nombre':nombre,
            'estado':d.get('estado'),
            'modo':d.get('modo'),
            'confianza':d.get('confianza'),
            'muestras_utilizables':d.get('muestras',0),
            'muestras_verificadas':d.get('muestras_verificadas',mem['verificados']),
            'observaciones_fuente':mem['observados_fuente'],
            'top5':[x.get('num') for x in d.get('ranking',[])[:5]]
        }
    return salida
...
@app.get('/api/historico/cargar',response_class=JSONResponse)
def api_historico_cargar(rd_paginas:int=4,esp_fechas:int=4):
    # Hard limits are also enforced in cargador_historico.py.
    return cargar_historico_incremental(rd_paginas,esp_fechas)

@app.get('/api/historico/estado',response_class=JSONResponse)
def api_historico_estado():
    return {
        'ultima':leer_estado('historico_ultima_carga'),
        'rd':leer_estado('historico_rd_ultima_carga'),
        'espana':leer_estado('historico_esp_ultima_carga'),
    }
...
def api_memoria(clave:str): return resumen_memoria_clave(clave)
...
def panel(clave,tipo):
    ...
    modo=d.get('modo','COMPLETO')
    confianza=d.get('confianza','MEDIA')
    etiqueta={'MEMORIA_MINIMA':'🟠 MEMORIA MÍNIMA','PROVISIONAL':'🟡 ESTUDIO PROVISIONAL','PROVISIONAL_AMPLIO':'🟡 ESTUDIO PROVISIONAL AMPLIO','COMPLETO':'🟢 ESTUDIO COMPLETO','MADURO':'🔵 MEMORIA MADURA'}.get(modo,modo)
    modo_html=f"<div class='box'><b>{etiqueta}</b><br>Datos utilizables: {d.get('muestras',0)} · Verificados: {d.get('muestras_verificadas',d.get('muestras',0))} · Confianza: {confianza}<br><small>Los datos observados permiten trabajar desde temprano; las alertas fuertes siguen usando evidencia verificada.</small></div>"
    alerta_html=''
    if tipo=='quiniela':
    ...
    return f"{modo_html}{alerta_html}{especial}<div class='box'><b>Memoria utilizable:</b> {d['muestras']} sorteos.</div>{bt_html}<table><tr><th>#</th><th>Número</th><th>Score</th><th>Razones</th></tr>{rows}</table>"
...
def index(sala:str|None=None):
    ...
    </style></head><body><div class='wrap'><h1>SHNEYDER IA PRO — ESTABLE REAL</h1><div class='tabs'>{tabs}</div><div class='box'><b>🧠 Estado:</b> {estado['resultados_totales']} resultados verificados · Última sincronización: {lasttxt}<br><b>🕒 Cortes RD:</b> 05:00 · 13:15 · 19:00. Antes de cada estudio se intenta actualizar la memoria.</div>{mejor_html}<div class='card'><h2>{nombre}</h2><p><a class='btn' href='/api/verificar/{clave}'>Verificar ahora</a></p>{body}</div></div></body></html>"""
