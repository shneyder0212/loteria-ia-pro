...
def analizar_quiniela(clave):
    ...
    if len(sorteos)<25:
        return {'estado':'SIN_DATOS','muestras':len(sorteos),'mensaje':'Se necesitan al menos 25 resultados verificados.'}
...
def analizar_kino(clave='kino_leidsa'):
    ...
    if len(sorteos)<30:
        return {'estado':'SIN_DATOS','muestras':len(sorteos),'mensaje':'Se necesitan al menos 30 resultados de Kino verificados.'}
