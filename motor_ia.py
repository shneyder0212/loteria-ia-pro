...
from memoria import obtener_resultados,obtener_resultados_trabajo,contar
...
def analizar_quiniela(clave):
    rows=obtener_resultados_trabajo(clave,3000)
    sorteos=[]
    for r in rows:
    ...
    if len(sorteos)==0:
        return {'estado':'SIN_DATOS','muestras':0,'mensaje':'Todavía no hay resultados observados ni verificados.'}

    verificados=contar(clave)
    if len(sorteos)<5:
        modo='MEMORIA_MINIMA'; confianza='MUY_BAJA'
    elif len(sorteos)<25:
        modo='PROVISIONAL'; confianza='BAJA'
    elif verificados<25:
        modo='PROVISIONAL_AMPLIO'; confianza='BAJA_MEDIA'
    elif len(sorteos)<80:
    ...
    return {
        ...
        'muestras_verificadas':verificados,
        'ultimo':sorteos[0],
        'ranking':ranking[:30],
...
def analizar_kino(clave='kino_leidsa'):
    rows=obtener_resultados_trabajo(clave,2500)
    sorteos=[]
    for r in rows:
        vals=[]
    ...
    if len(sorteos)==0:
        return {'estado':'SIN_DATOS','muestras':0,'mensaje':'Todavía no hay resultados de Kino observados ni verificados.'}
    verificados=contar(clave)
    if len(sorteos)<10:
        modo='MEMORIA_MINIMA'; confianza='MUY_BAJA'
    elif len(sorteos)<30 or verificados<20:
        modo='PROVISIONAL'; confianza='BAJA'
...
def analizar_bolas(clave,max_num,cantidad):
    ...
    if len(sorteos)<20:
        return {'estado':'SIN_DATOS','muestras':len(sorteos),'mensaje':'Se necesitan al menos 20 sorteos verificados.'}
    ...
    return {'estado':'OK','muestras':len(sorteos),'ranking':rank[:30],'seleccion':[x['num'] for x in rank[:cantidad]]}
